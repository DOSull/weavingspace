...
| Date       | Changes
|:-          |:-------
| 2026-04-17 | Corrected images and other links.
...
+ ['Zig-zag' an edge](https://dosull.github.io/weavingspace/doc/weavingspace/topology.html#Topology.zigzag_edge), which will add [corners](#corner) along an edge in a zig-zag pattern.
+ [Rotate an edge](https://dosull.github.io/weavingspace/doc/weavingspace/topology.html#Topology.rotate_edge) about its centre, which may cause its associated end point [vertices](#vertex) to move.
+ ['Push' a vertex](https://dosull.github.io/weavingspace/doc/weavingspace/topology.html#Topology.push_vertex) will move a [vertex](#vertex) in the direction of the resultant unit vector of its incoming incident [edges](#edge).
+ [Nudge a vertex](https://dosull.github.io/weavingspace/doc/weavingspace/topology.html#Topology.nudge_vertex) will move all [vertices](#vertex) in a specifed [transitivity class](#transitivity-class) by a given displacement. This almost always makes tiles non-similar.

Some examples of these can be seen in [this notebook](https://github.com/DOSull/weavingspace/blob/main/examples/topology-working.ipynb), although a less impenetrable interface to applying transformations is likely required.

It's also worth noting that the `Topology` class offers a likely more reliable way to generate dual tilings, than the [`tiling_utils.get_dual_tile_unit_`](https://dosull.github.io/weavingspace/doc/weavingspace/tiling_utils.html#get_dual_tile_unit) function.
...
The symmetry now works, and can accurately detect the [symmetries of a tile](https://github.com/DOSull/weavingspace/blob/main/examples/symmetry-working.ipynb), and of a [tiling](https://github.com/DOSull/weavingspace/blob/main/examples/topology-working.ipynb).

As seen in the [topology notebook](https://github.com/DOSull/weavingspace/blob/main/examples/topology-working.ipynb) we can now detect and display all the symmetries of a tiling (probably reliably).
...
### Fundamental unit
A subset of a tiling which under two non-parallel [translations](#translation) can tile the plane. It is important to realise that the fundamental unit of a tiling is not uniquely defined. In a tiling with two translation vectors, for example, any parallelogram shaped region defined by those two vectors is a fundamental unit. In our implementation approximately equivalent to a [`Tileable`](#tileable).
...
### Symmetry
OMG. All of mathematics. But seriously... albeit briefly, in our context a symmetry is any [transformation](#transformation) of a tiling that maps its [elements](#element) back on to other matching elements. Tiles also have symmetries but the symmetries of a tiling and of its constituent tiles are not the same. For example in the Cairo tiling (Laves 3.3.4.3.4 in the figure above), the pentagonal tiles have only a single reflection symmetry (also a symmetry of the tiling), but the tiling also has 90&deg; rotational symmetries.
